...
pub fn ctl_looked_op0<F: Field>() -> Vec<Column<F>> {
    Column::singles(columns::INPUT_REGISTER_0).collect()
}
pub fn ctl_looked_op1<F: Field>() -> Vec<Column<F>> {
    Column::singles(columns::INPUT_REGISTER_1).collect()
}
pub fn ctl_looked_res<F: Field>() -> Vec<Column<F>> {
    Column::singles(columns::OUTPUT_REGISTER).collect()
}


pub fn ctl_looked_mult<F: Field>() -> Vec<Column<F>> {
    let indices = [columns::INPUT_REGISTER_0, columns::INPUT_REGISTER_1, columns::OUTPUT_REGISTER];
...
pub fn ctl_looked_filter<F: Field>() -> Column<F> {
    Column::single(columns::IS_ADD)
}

pub fn ctl_looked_filter_mult<F: Field>() -> Column<F> {
    Column::single(columns::IS_MUL)
}
...
impl<F: RichField, const D: usize> ArithmeticStark<F, D> {
    ...
    pub(crate) fn generate_trace(&self, operations: Vec<Operation>) -> Vec<PolynomialValues<F>> {
        ...
        for op in operations {
            let (mut row1, maybe_row2) = op.to_rows();
