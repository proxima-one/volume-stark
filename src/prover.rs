...
use crate::generation::{generate_traces, GenerationInputs, PatriciaInputs};
    ...
    [(); SearchStark::<F, D>::COLUMNS]:,
{
    let (traces, public_values) = timed!(
        timing,
        "generate all traces",
        generate_traces(all_stark, inputs, patricia_input, config, timing)?
    );
    ...
    [(); SearchStark::<F, D>::COLUMNS]:,
{
    ...
    let data_proof = timed!(
        ...
        prove_single_table(
            ...
            config,
            &trace_poly_values[Table::Data as usize],
            &trace_commitments[Table::Data as usize],
            &ctl_data_per_table[Table::Data as usize],
            challenger,
            timing,
        )?
    );
    ...
    let sum_proof = timed!(
        ...
        prove_single_table(
            &all_stark.sum_stark,
            config,
            &trace_poly_values[Table::Sum as usize],
            &trace_commitments[Table::Sum as usize],
    ...
    let search_proof = timed!(
        ...
        prove_single_table(
            ...
            config,
            &trace_poly_values[Table::Search as usize],
            &trace_commitments[Table::Search as usize],
            &ctl_data_per_table[Table::Search as usize],
            challenger,
            timing,
        )?
    );
