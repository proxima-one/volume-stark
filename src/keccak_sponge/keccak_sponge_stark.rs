...
use super::keccak_util::keccakf_u32s;
...
pub(crate) fn ctl_looked_data<F: Field>() -> Vec<Column<F>> {
    ...
    // .collect()
    //outputs
    let hash_cols = cols.updated_state_bytes;
    Column::singles(hash_cols.to_vec()).collect()
}
...
pub(crate) fn ctl_looking_data<F: Field>() -> Vec<Column<F>> {
    let cols = KECCAK_SPONGE_COL_MAP;
