        ...
        [(); SearchStark::<F, D>::COLUMNS]:,
{
    ...
    verify_stark_proof_with_challenges(
        data_stark,
        &all_proof.stark_proofs[Table::Data as usize].proof,
        &stark_challenges[Table::Data as usize],
        &ctl_vars_per_table[Table::Data as usize],
        config,
    )?;
    verify_stark_proof_with_challenges(
        sum_stark,
        &all_proof.stark_proofs[Table::Sum as usize].proof,
        &stark_challenges[Table::Sum as usize],
        &ctl_vars_per_table[Table::Sum as usize],
        config,
    )?;
    verify_stark_proof_with_challenges(
        search_stark,
    ...
    for c in 0..config.num_challenges {
        extra_looking_products[Table::Data as usize][c] = get_data_extra_looking_products(
            &public_values,
...
pub(crate) fn get_data_extra_looking_products<F, const D: usize>(
...
{
    let mut prod = F::ONE;
    let mut row = vec![F::ZERO; 8];
    for (i, limb) in public_values.starting_blockhash.into_uint().0.into_iter().enumerate() {
        row[2 * i] = F::from_canonical_u32(limb as u32);
        row[2 * i + 1] = F::from_canonical_u32((limb >> 32) as u32);
    }
